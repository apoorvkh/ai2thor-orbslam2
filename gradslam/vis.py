...
def pointcloud_updates(intermediate_pcs, poses, K, ms_per_frame=50):
    """
    Args:
        - intermediate_pcs (List[plotly]): list of pointcloud plotly objects, each of batch size 1
    ...
    # visualization
    traj_frustums = plotly_poses(poses, K)
    data = [[*frustum, scatter3d] for frustum, scatter3d in zip(traj_frustums, intermediate_pcs)]

    steps = [
        {"args": [[i], frame_args(0)], "label": i, "method": "animate"}
        for i in range(len(intermediate_pcs))
    ]
    sliders = [
        {
            "active": 0,
